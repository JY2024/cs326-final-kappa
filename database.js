...
export async function createUserObj(username, password, displayName) {
    const ret = await client.query(
        "SELECT username FROM user_T WHERE username=$1;", [username]
    );
    if(ret.rows === null && password.length >= 8){
        ...
        return JSON.stringify({status: 'ERROR'});//, msg: 'Password must contain 8 or more characters'});
    }
    else{
        return JSON.stringify({status: 'ERROR'});//, msg: 'User ' + username + ' already exists'});
    }
